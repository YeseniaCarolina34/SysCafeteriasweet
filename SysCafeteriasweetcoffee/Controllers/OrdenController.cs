...
namespace SysCafeteriasweetcoffee.Controllers
{
    ...
    public class OrdenController : Controller
    {
        ...
        [HttpPost]
        public IActionResult AgregarOrden([FromBody] OrdenRequest request) // se creó la clase OrdenRequest
        {
            if (request == null || request.DetallesDeOrden == null || request.DetallesDeOrden.Count == 0)
            {
                return BadRequest("La orden no contiene detalles válidos.");
            }
            ...
            foreach (var detalle in request.DetallesDeOrden)
            {
